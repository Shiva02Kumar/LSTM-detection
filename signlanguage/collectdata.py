...
while True:
    _,frame=cap.read()
    count = {
             '1': len(os.listdir(directory+"/One"))
            #  '2': len(os.listdir(directory+"/Two")),
            #  '3': len(os.listdir(directory+"/Three")),
    ...
    if interrupt & 0xFF == ord('1'):
        cv2.imwrite(directory+'One/'+str(count['1'])+'.png',frame)
    if interrupt & 0xFF == ord('2'):
        cv2.imwrite(directory+'Two/'+str(count['2'])+'.png',frame)
    if interrupt & 0xFF == ord('3'):
        cv2.imwrite(directory+'Three/'+str(count['3'])+'.png',frame)
